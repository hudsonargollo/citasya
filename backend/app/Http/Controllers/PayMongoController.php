...
class PayMongoController extends ParentBookingController
{
    ...
    public function processing(Request $request, int $bookingId, string $paymentMethodId): RedirectResponse
    {
    ...
    public function success(Request $request, int $bookingId, string $paymentIntentId): RedirectResponse
    {
        $this->booking = $this->bookingRepository->findWithoutFail($bookingId);
        if (!empty($this->booking)) {
